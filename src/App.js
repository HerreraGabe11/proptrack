...
function Spark({ data, keys, colors, height = 120 }) {
  ...
  const gx=(i)=>(i/(data.length-1))*(w-80)+55;
  const ticks=5;const yLabels=[];for(let t=0;t<ticks;t++){const val=mn+(mx-mn)*(t/(ticks-1));yLabels.push({y:gy(val),label:"$"+(val>=1000000?(val/1000000).toFixed(1)+"M":(val/1000).toFixed(0)+"K")});}
  return (<svg viewBox={"0 0 "+w+" "+h} style={{width:"100%",height}} preserveAspectRatio="none">{yLabels.map((yl,i)=><g key={"yl"+i}><line x1={28} y1={yl.y} x2={w-5} y2={yl.y} stroke="#2d3748" strokeWidth="0.5" strokeDasharray="4,4"/><text x={26} y={yl.y+4} textAnchor="end" fontSize="10" fill="#6b7280">{yl.label}</text></g>)}{keys.map((key,ki)=>{const pts=data.map((d,i)=>gx(i)+","+gy(d[key]||0)).join(" ");return(<g key={key}><polyline points={pts} fill="none" stroke={colors[ki]} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>{data.map((d,i)=><circle key={i} cx={gx(i)} cy={gy(d[key]||0)} r="4" fill={colors[ki]}/>)}</g>);})}{data.map((d,i)=><text key={i} x={gx(i)} y={h-1} textAnchor="middle" fontSize="14" fill="#8896ab">{d.month}</text>)}</svg>);
}
